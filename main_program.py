...
try:
    camera1_thread = threading.Thread(target=gi.FFmpegImageCapture, args=("camera1", "10.193.1.49:5000"), daemon=True)
    camera1_thread.start()

    camera2_thread = threading.Thread(target=gi.FFmpegImageCapture, args=("camera2", "10.193.9.51:5000"), daemon=True)
    camera2_thread.start()

    # time.sleep(2)

    # image1_thread = threading.Thread(target=pi.ProcessImages, args=(3,0, "camera1"), daemon=True)
    # image1_thread.start()

    # image1_thread = threading.Thread(target=pi.ProcessImages, args=(4,1, "camera2"), daemon=True)
    # image1_thread.start()
